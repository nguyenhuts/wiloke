...
<?php
			...
			if ( ! empty( $post['postmeta'] ) ) {
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						...
						if ( ! $value )
						{
							// $meta['value'] = $this->fix_corrupted_serialized_string($meta['value']);
							$value = maybe_unserialize( $meta['value'] );
						}
	...
	function process_menu_item( $item ) {
		...
		$_menu_item_classes =  $this->fix_corrupted_serialized_string($_menu_item_classes);
